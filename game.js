...
// Zone helper
// ---------------------------------------------------------------------
// map.js stores zones as an OBJECT:
//
// zones: {
//     counter: { ... },
//     machine: { ... },
...
// }
//
// Some versions of the game may use an ARRAY instead.
//
// This helper supports both formats so the rest of game.js can always
// safely work with an array.
function getZones(scene) {
    ...
    // Object keyed by zone type
    if (typeof scene.zones === 'object') {
        return Object.entries(scene.zones).map(([key, zone]) => ({
            ...zone,
            type: zone.type || key
        }));
    }
...
function rebuildCustomers() {
    const counters = getZones(scene).filter(z => z.type === 'counter');
...
for (const sceneKey in mapConfig.scenes) {
    ...
    const docks = getZones(s).filter(z => z.type === 'dock');

    for (const dock of docks) {
        const initial = Math.min(3, dock.maxStock || 5);

        for (let i = 0; i < initial; i++) {
            spawnAtDock(sceneKey, dock);
        }
    }
}
...
function getSolids() {
    const zoneSolids = getZones(scene).filter(z => z.solid);

    const solidWalls = (scene.walls || []).filter(
        w => w.solid !== false
    );
...
function getNpcLines(npc) {
    ...
    if (q && q.enabled) {
        ...
        if (
            player.heldItem &&
            player.heldItem.type === q.itemNeeded
        ) {
...
function renderDialogueBox() {
    ...
    document.getElementById('dialogueText').textContent =
        dialogue.lines[dialogue.lineIndex] || '...';
}
...
function update() {
    ...
    // Get the different zone types from the current scene.
    // getZones() supports both arrays and objects.
    const counterZones = getZones(scene).filter(
        z => z.type === 'counter'
    );

    const machineZones = getZones(scene).filter(
        z => z.type === 'machine'
    );

    const dockZones = getZones(scene).filter(
        z => z.type === 'dock'
    );
    ...
    for (const dock of dockZones) {
        if (dock._nextSpawnAt === undefined) {
            dock._nextSpawnAt =
                now + (dock.spawnIntervalMs || 4000);
        }

        if (now >= dock._nextSpawnAt) {
            const stockNearby = items.filter(it =>
                it.scene === currentSceneKey &&
                it.type === (dock.spawnItem || 'box') &&
...
function drawZones() {
    // FIXED: scene.zones may be an object rather than an array.
    for (const z of getZones(scene)) {
...
function draw() {
    ...
    for (const npc of (scene.npcs || [])) {
        ...
        if (
            npc.quest &&
            npc.quest.enabled &&
            npc.quest.amountDelivered <
                npc.quest.amountNeeded
        ) {
            ctx.fillStyle = '#f1c40f';
